...
class TestGetCurrentUserId:
    async def test_valid_token_returns_uuid(self, mock_db):
        ...
        with patch("app.modules.tasks.dependencies.get_db", return_value=mock_db), \
             patch("app.modules.tasks.dependencies.settings") as mock_settings:
    ...
    async def test_invalid_token_raises_401(self, mock_db):
        ...
        with patch("app.modules.tasks.dependencies.get_db", return_value=mock_db), \
             patch("app.modules.tasks.dependencies.settings") as mock_settings, \
             patch("app.modules.tasks.dependencies.jwt.decode", side_effect=jwt.InvalidTokenError("bad token")):
            mock_settings.JWT_SECRET_KEY = "test_secret"
    ...
    async def test_expired_token_raises_401(self, mock_db):
        ...
        with patch("app.modules.tasks.dependencies.get_db", return_value=mock_db), \
             patch("app.modules.tasks.dependencies.settings") as mock_settings, \
             patch("app.modules.tasks.dependencies.jwt.decode", side_effect=jwt.ExpiredSignatureError("expired")):
            mock_settings.JWT_SECRET_KEY = "test_secret"
            with pytest.raises(HTTPException) as exc_info:
                get_current_user_id(credentials)
            assert exc_info.value.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    async def test_malformed_uuid_sub_raises_401(self, mock_db):
        ...
        with patch("app.modules.tasks.dependencies.get_db", return_value=mock_db), \
             patch("app.modules.tasks.dependencies.settings") as mock_settings, \
             patch("app.modules.tasks.dependencies.jwt.decode", return_value={"sub": "not-a-uuid"}):
            mock_settings.JWT_SECRET_KEY = "test_secret"
            with pytest.raises(HTTPException) as exc_info:
                get_current_user_id(credentials)
            assert exc_info.value.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestGetAttachmentService:
    async def test_returns_attachment_service(self, mock_db):
        with patch("app.modules.tasks.dependencies.AttachmentRepository") as mock_repo_cls, \
             patch("app.modules.tasks.dependencies.get_storage_service") as mock_storage_fn:
...
class TestGetTasksService:
    async def test_returns_task_service(self, mock_db):
        mock_attachment_service = MagicMock()
        with patch("app.modules.tasks.dependencies.TaskRepository") as mock_repo_cls, \
             patch("app.modules.tasks.dependencies.get_attachment_service", return_value=mock_attachment_service):
            mock_repo_cls.return_value = MagicMock()
            result = await get_tasks_service(mock_db, mock_attachment_service)
            assert isinstance(result, TaskService)

    async def test_task_service_has_repo_and_attachment_service(self, mock_db):
        mock_attachment_service = MagicMock()
        with patch("app.modules.tasks.dependencies.TaskRepository") as mock_repo_cls, \
             patch("app.modules.tasks.dependencies.get_attachment_service", return_value=mock_attachment_service):
            mock_repo = MagicMock()
